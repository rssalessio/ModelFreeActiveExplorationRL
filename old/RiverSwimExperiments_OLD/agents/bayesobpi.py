...
class BayesOBPI(Agent):
    ...
    def suggested_exploration_parameter(dim_state: int, dim_action: int) -> float:
        return 1 #/ (3*dim_state)#max(1, 1 / (2*dim_state))
    ...
    def process_experience(self, experience: Experience, step: int) -> None:
        ...
        if np.random.uniform() < 1e-3:
            x = (np.random.random(self.Q.shape) * (self.Q==self.Q.max(-1, keepdims=True))).argmax(-1)
            print(np.median(x,axis=0).astype(int))
            print(self.Q.mean(0).argmax(-1))
            # import pdb
            # pdb.set_trace()
        ...
        x = (np.random.random(self.Q.shape) * (self.Q==self.Q.max(-1, keepdims=True))).argmax(-1)
        self.greedy_policy  = np.median(x,axis=0).astype(int)
