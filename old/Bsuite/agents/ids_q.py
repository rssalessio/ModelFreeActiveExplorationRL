...
class EnsembleQ(nn.Module):
    def __init__(self, input_size: int, output_size: int, ensemble_size: int, hidden_size: int = 50):
        super().__init__()
...
def default_agent(
    ...
    optimizer = torch.optim.Adam(ensemble.parameters(), lr=5e-2)
    optimizer_quantile_net = torch.optim.Adam(quantile_net.parameters(), lr = 1e-2)
