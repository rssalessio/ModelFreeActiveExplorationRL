...
class BootstrappedDqn(Agent):
    ...
    def _step(self, transitions: Sequence[torch.Tensor]):
        ...
        # import pdb
        # pdb.set_trace()
        loss = torch.mul(torch.square(q_values - target_y.detach()), m_t).mean() #* self._num_ensemble
        #loss = nn.HuberLoss()(q_values, target_y.detach()) / self._num_ensemble
        loss.backward()
    ...
    def update(
        ...
        return self._update(
            np.float32(timestep.observation), action, np.float32(new_timestep.reward),
    ...
    def _update(
        ...
        if done:
            ...
            print(f'New head: {self._active_head} - steps {self._total_steps}')
            with torch.no_grad():
                ...
                print(f'Mu: {qvalues.mean(1).mean(0)} - std: {qvalues.std(1).mean(0) + qvalues.mean(1).std(0)} - prior: {qprior.mean(1).mean(0)} ')
...
class EnsembleWithPrior(nn.Module):
    def __init__(self, input_size: int, output_size: int, prior_scale: float, ensemble_size: int, hidden_size: int = 50):
        super().__init__()
...
def default_agent(
    ...
    return BootstrappedDqn(
        ...
        mask_prob=.5,
        noise_scale=0.0,
        epsilon_fn=lambda t: max(0.05, 10 / (10 + t)),
        seed=seed,
    )