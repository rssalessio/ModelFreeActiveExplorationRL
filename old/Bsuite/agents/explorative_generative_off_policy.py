...
class ExplorativeAgent(Agent):
    ...
    def update(
        ...
        return self._update(
            np.float32(timestep.observation).flatten(), action, np.float32(new_timestep.reward),
            np.float32(new_timestep.observation).flatten(), 1-new_timestep.discount)
    ...
    def _update(
        ...
        if done:
            ...
            with torch.no_grad():
                qvalues = self._ensemble.forward(self._pool_states).q_values
                # qprior = self._ensemble._prior_network(self._pool_states[None, ...].repeat(self._num_ensemble, 1, 1)).swapaxes(0,1)
                print(f'Mu: {qvalues.mean(1).mean(0)} - std: {qvalues.std(1).mean(0) + qvalues.mean(1).std(0)} ')
