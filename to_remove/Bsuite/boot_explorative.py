...
class BootstrappedExplorative(base.Agent):
  ...
  def __init__(
      ...
      ensemble_Q: Sequence[snt.Module],
      ensemble_M: Sequence[snt.Module],
      batch_size: int,
      discount: float,
      replay_capacity: int,
      min_replay_size: int,
      ...
      optimizer_Q: snt.Optimizer,
      optimizer_M: snt.Optimizer,
      mask_prob: float,
      noise_scale: float,
    ...
    """Bootstrapped DQN with additive prior functions."""
    # Agent components.
    self._Q_ensemble = ensemble_Q
    self._M_ensemble = ensemble_M
    self._Q_forward = [tf.function(net) for net in ensemble_Q]
    self._M_forward = [tf.function(net) for net in ensemble_M]
    self._Q_target_ensemble = [copy.deepcopy(network) for network in ensemble_Q]
    self._num_ensemble = len(ensemble_Q)
    self._Q_optimizer = optimizer_Q
    self._M_optimizer = optimizer_M
    self._replay = replay.Replay(capacity=replay_capacity)

    # Create variables for each network in the ensemble
    for network in ensemble_Q:
      snt.build(network, (None, *obs_spec.shape))
      
    for network in ensemble_M:
      snt.build(network, (None, *obs_spec.shape))
  ...
  def _step(self, transitions: Sequence[tf.Tensor]):
    ...
    o_tm1, a_tm1, r_t, d_t, o_t, m_q_t, z_t, m_v_t = transitions
    variables = tree.flatten(
        [model.trainable_variables for model in self._Q_ensemble])
    with tf.GradientTape() as tape:
      losses = []
      for k in range(self._num_ensemble):
        net = self._Q_ensemble[k]
        target_net = self._Q_target_ensemble[k]
        ...
        loss = tf.square(train_value - target_y) * m_q_t[:, k]
        losses.append(loss)

      loss = tf.reduce_mean(tf.stack(losses))
      gradients = tape.gradient(loss, variables)
    self._total_steps.assign_add(1)
    self._Q_optimizer.apply(gradients, variables)

    # Periodically update the target network.
    if tf.math.mod(self._total_steps, self._target_update_period) == 0:
      for k in range(self._num_ensemble):
        for src, dest in zip(self._Q_ensemble[k].variables,
                             self._Q_target_ensemble[k].variables):
          dest.assign(src)
    
    ########## Train M network
    variables = tree.flatten(
        [model.trainable_variables for model in self._M_ensemble])
    with tf.GradientTape() as tape:
      losses = []
      for k in range(self._num_ensemble):
        Qnet = self._Q_ensemble[k]
        Mnet = self._M_ensemble[k]
        ...
        one_hot_actions = tf.one_hot(a_tm1, depth=self._num_actions)
        train_value = tf.reduce_sum(q_values * one_hot_actions, axis=-1)
        target_value = tf.stop_gradient(tf.reduce_max(Qnet(o_t), axis=-1))
        target_y = tf.stop_gradient(tf.square((r_t + z_t[:, k] + self._discount * d_t * target_value - train_value) / self._discount))
        
        m_values = Mnet(o_tm1)
        m_train_value = tf.reduce_sum(m_values * one_hot_actions, axis=-1)
        loss = tf.square(target_y-m_train_value ) * m_v_t[:, k]
        losses.append(loss)

      loss = tf.reduce_mean(tf.stack(losses))
      gradients = tape.gradient(loss, variables)
    self._total_steps.assign_add(1)
    self._M_optimizer.apply(gradients, variables)
  ...
  def select_action(self, timestep: dm_env.TimeStep) -> base.Action:
    ...
    q_values = self._Q_forward[self._active_head](batched_obs)[0].numpy()
    m_values = np.clip(self._M_forward[self._active_head](batched_obs)[0].numpy(), 0, np.inf)
    
    delta = q_values.max() - q_values
    idxs = np.isclose(delta,0)
    delta[idxs] = delta[~idxs].min() * (1-self._discount)
  ...
  def update(
    ...
    self._replay.add(
        TransitionWithMaskAndNoise(
            ...
            m_q_t=self._rng.binomial(1, self._mask_prob,
                                   self._num_ensemble).astype(np.float32),
            m_v_t=self._rng.binomial(1, self._mask_prob,
                                   self._num_ensemble).astype(np.float32),
...
def make_ensemble(num_actions: int,
                  ...
                  num_units: int = 50,
                  prior_scale: float = 3.,
                  activate_final: bool = False) -> Sequence[snt.Module]:
  ...
  for _ in range(num_ensemble):
    network = snt.Sequential([
        snt.Flatten(),
        snt.nets.MLP(output_sizes, activate_final=activate_final)
    ])
...
def default_agent(
    ...
    action_spec: specs.DiscreteArray,
    num_ensemble: int = 20,
) -> BootstrappedExplorative:
  """Initialize a Bootstrapped DQN agent with default parameters."""
  ensemble_Q= make_ensemble(
      num_actions=action_spec.num_values, num_ensemble=num_ensemble, activate_final=False)
  ensemble_M = make_ensemble(
      num_actions=action_spec.num_values, num_ensemble=num_ensemble, activate_final=True)
  optimizer_Q = snt.optimizers.Adam(learning_rate=1e-3)
  optimizer_M = snt.optimizers.Adam(learning_rate=5e-4)
  return BootstrappedExplorative(
      obs_spec=obs_spec,
