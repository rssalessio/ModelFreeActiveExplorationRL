...
class BayesOBPI(Agent):
    ...
    def __init__(self, parameters: BayesOBPIParameters, agent_parameters: AgentParameters):
        ...
        self.combined_M = lambda T: self.combined_value(self.M, T,2 * self.parameters.kbar)#self.M + self.prior_M * self.parameters.prior_m_scale*np.sqrt(np.log(self.ns * self.na * (T + 1))/(1+self.state_action_visits))#/ ((1 - self.discount_factor) ** (2 * self.parameters.kbar))
