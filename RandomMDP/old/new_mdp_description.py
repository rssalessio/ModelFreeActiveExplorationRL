...
class MDPDescription2(MDPDescription):
    ...
    def evaluate_allocation(self, omega: npt.NDArray[np.float64], type: BoundType = BoundType.BOUND_1, navigation_constraints: bool = False) -> float:
        ...
        if type == BoundType.BOUND_1:
            ...
            obj_supp1 = []
            for s in range(ns):
                obj_supp1.append(Hstar / omega[s, self.pi_greedy[s]] )
            ...
            return (np.max(obj_supp) + np.max(obj_supp1)) / self.normalizer
        else:
    ...
    def compute_allocation(
        ...
        def upper_bound_eq_6():
            ...
            obj_supp_1 = []
            for s in range(ns):
                obj_supp_1.append(self.normalizer * cp.inv_pos(omega[s, pi_greedy[s]]) * Hstar)
            ...
            return cp.max(cp.hstack(obj_supp)) + cp.max(cp.hstack(obj_supp_1))
        solver = cp.MOSEK
        for _it in range(10):
            try:
                if type == BoundType.BOUND_1:
    ...
    def compute_policy_v1(
        ...
        # policy = policy / policy.sum(1)[:, np.newaxis]
            ...
            def upper_bound_eq_5(s: int):
                ...
                for a in range(na):
                    if a == self.pi_greedy[s]: continue
                    T1 = self.normalizer * cp.inv_pos(omega[a]) * (2 + 8 * golden_ratio_sq * Mk[s,a]) / (Delta_sq[s,a]+tol)
                    
                    T2 = self.normalizer * cp.inv_pos(omega[pi_greedy[s]]) * (2 + 8 * golden_ratio_sq * Mk[s, pi_greedy[s]]) / (tol + Delta_sq[s,a] * ((1 - self.discount_factor) ** 2))
    ...
    def compute_optimal_soft_policy(self, discount_explorative: float, theta: float):
        ...
        Rnew = np.maximum(self.delta_sq, min_delta) / (2 + 8 *golden_ratio_sq * self.Mk_V_greedy)
